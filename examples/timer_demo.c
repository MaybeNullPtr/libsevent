...
static void on_tick(void *data)
{
    ...
    if (drift > t->max_drift) t->max_drift = drift;
    ...
    if (is_first)
        ...
        printf("  %5s  %5ldms     %dms     %ldms   %+ldms",
               t->name, elapsed, t->interval, actual_interval, drift);
...
int main(void)
{
    ...
    struct ticker fast = {
        .ctx = ctx, .name = "fast", .interval = 200, .limit = 0,
        .last_fire = t0,
    };
    struct ticker medium = {
        .ctx = ctx, .name = "med ", .interval = 500, .limit = 0,
        .last_fire = t0,
    };
    struct ticker slow = {
        .ctx = ctx, .name = "slow", .interval = 1000, .limit = 5,
        .last_fire = t0,
    };
