...
static void send_response(struct http_conn *c, const char *body)
{
    ...
    ssize_t r;
    r = write(c->fd, g_status, strlen(g_status)); if (r < 0) goto close;
    r = write(c->fd, g_headers, strlen(g_headers)); if (r < 0) goto close;
    r = write(c->fd, body, strlen(body)); (void)r;
...
static void on_client_read(void *data)
{
    ...
    /* 根据路径返回不同内容 */
    if (strcmp(path, "/") == 0) {
        send_response(c, "Hello from libsevent HTTP server!\n"
