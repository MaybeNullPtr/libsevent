...
static void parse_args(int argc, char **argv)
{
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(argv[i],"--clients")==0 && i+1<argc) g_ncli = atoi(argv[++i]);
        else if (strcmp(argv[i],"--size")==0 && i+1<argc)    g_size = atoi(argv[++i]);
        else if (strcmp(argv[i],"--count")==0 && i+1<argc)   g_nmsg = atoi(argv[++i]);
...
int main(int argc, char **argv)
{
    ...
    if (!g_c) { perror("calloc"); return 1; }

    sevent_context *ctx = sevent_create();
    if (!ctx) { fprintf(stderr,"create fail\n"); return 1; }

    int ok = 0;
    for (int i = 0; i < g_ncli; i++) {
        g_c[i].buf = malloc((size_t)g_size);
        if (!g_c[i].buf) { perror("malloc"); return 1; }

        int fd = tcp_connect(g_host, g_port);
        if (fd < 0) { fprintf(stderr,"  conn[%d] fail\n",i); g_fin++; continue; }
        ok++;
        ...
        struct sevent_io_handler h = {
            .fd = fd, .io_write = on_connect, .data = &g_c[i],
        };
    ...
    for (int i = 0; i < g_ncli; i++) free(g_c[i].buf);
    free(g_c);
    sevent_destroy(ctx);
    return g_err ? 1 : 0;
}
