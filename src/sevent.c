...
static void run_free_death(sevent_context *ctx) {
    ...
    struct sevent_io *die_io     = ctx->death_io;
    ctx->death_io                = NULL;
    struct sevent_timer *die_tmr = ctx->death_timer;
    ctx->death_timer             = NULL;
...
static void run_timers(sevent_context *ctx, bool has_timer, long delta, bool *fired) {
    ...
    for(struct sevent_timer *t = ctx->timer_list; t; t = t->next) {
        ...
        int fire_count  = 0;
        while(t->remaining_ms <= 0 && fire_count < MAX_FIRE_PER_TIMER) {
            fire_count++;
