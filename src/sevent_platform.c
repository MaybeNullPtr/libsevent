...
int sevent_mutex_init(sevent_mutex_t *m) { return pthread_mutex_init(m, NULL); }

int sevent_mutex_lock(sevent_mutex_t *m) { return pthread_mutex_lock(m); }

int sevent_mutex_unlock(sevent_mutex_t *m) { return pthread_mutex_unlock(m); }

int sevent_mutex_destroy(sevent_mutex_t *m) { return pthread_mutex_destroy(m); }
...
int sevent_wakeup_pair(int fds[2]) {
    ...
    /* ---- Level 3: UDP loopback (通用降级) ---- */
    {
        ...
        addr.sin_port        = 0;
        if(bind(rd, (struct sockaddr *)&addr, sizeof(addr)) < 0)
            goto fail3;
        if(getsockname(rd, (struct sockaddr *)&addr, &addrlen) < 0)
            goto fail3;
