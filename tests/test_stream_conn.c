...
/* 用例统一收尾: 销毁残余 server 连接 → 关 listener */
...
/* 启动 echo server, 返回实际监听端口 (随机端口) */
static int start_server(sevent_context *ev) {
    g_lfd = socket(AF_INET, SOCK_STREAM, 0);
    if(g_lfd < 0)
        return -1;
    int on = 1;
    (void)setsockopt(g_lfd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on));
    struct sockaddr_in a;
    memset(&a, 0, sizeof(a));
    a.sin_family = AF_INET;
    a.sin_port   = 0;
    if(bind(g_lfd, (struct sockaddr *)&a, sizeof(a)) < 0 || listen(g_lfd, 8) < 0)
        return -1;
    socklen_t al = sizeof(a);
    if(getsockname(g_lfd, (struct sockaddr *)&a, &al) < 0)
        return -1;
    sevent_io_register(ev, &(sevent_io_handler){.fd = g_lfd, .io_read = on_accept, .data = NULL});
...
static void on_accept_close(void *d) {
    (void)d;
    int fd = accept(g_lfd, NULL, NULL);
    if(fd >= 0) {
        close(fd); /* 立即关闭 */
...
static int t_tcp_write_error(void) {
    ...
    g_cli_close_on_data    = 0;
    g_srv_closed           = 0;
    /* raw listen (不注册 tcp_conn 服务端): accept 回调立即关闭连接 */
    g_lfd                  = socket(AF_INET, SOCK_STREAM, 0);
    if(g_lfd < 0)
        return 1;
    int on = 1;
    ...
    sevent_io_register(ev, &(sevent_io_handler){.fd = g_lfd, .io_read = on_accept_close, .data = NULL});
    g_ev_open = g_ev_error        = 0;
    g_ev_error_code               = 0;
    sevent_stream_conn_config cfg = {0};
    sevent_stream_conn       *s   = sevent_stream_create(ev, &cfg);
