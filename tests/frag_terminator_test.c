...
static int srv_send_frame(int fd, int fin, int opcode, const uint8_t *payload, uint64_t len) {
    uint8_t hdr[16];
...
static void on_srv_read(void *data) {
    ...
    if(g_srv.state == S_HANDSHAKE) {
        ...
        if(srv_send_frame(g_srv.client_fd, 0, WS_OPCODE_TEXT, (const uint8_t *)TEXT_DATA, TEXT_LEN) < 0)
            goto fail;
        ...
        /* 帧 2: CONT (fin=0, len=248) — 总和 = 256 = recv_cap */
        {
            ...
            if(srv_send_frame(g_srv.client_fd, 0, WS_OPCODE_CONT, fill, CONT_LEN) < 0)
                goto fail;
        }
    ...
    if(g_srv.state == S_WAIT_ECHO) {
        ...
        while(pos < g_srv.len) {
            ...
            if((hdr.opcode == WS_OPCODE_TEXT || hdr.opcode == WS_OPCODE_BINARY) && hdr.fin) {
                /* 验证回显 */
                if((size_t)hdr.payload_len != RECV_BUF_SIZE || memcmp(payload, TEXT_DATA, TEXT_LEN) != 0)
                    goto echo_fail;
...
static void on_ws_message(void *data, const void *m, size_t l, bool bin, bool fin, uint64_t total) {
    (void)data;
    (void)bin;
    (void)total;
...
static void on_ws_close(void *data, uint16_t code, const char *reason, size_t rl) {
    (void)data;
    (void)code;
    (void)reason;
    (void)rl;
...
int main(void) {
    g_ctx = sevent_create();
    if(!g_ctx) {
        fprintf(stderr, "FAIL: create ctx\n");
        return 1;
    }
