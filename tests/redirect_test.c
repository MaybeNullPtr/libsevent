...
close:
    if(g_redir_io) { sevent_io_unregister(g_ctx, g_redir_io); g_redir_io = NULL; }
    close(fd);
}
...
static void on_echo_read(void *data) {
    (void)data;
    if(g_echo_fd <= 0) return;
    ssize_t n = read(g_echo_fd, g_echo_buf + g_echo_len, sizeof(g_echo_buf) - g_echo_len);
    if(n < 0 && (errno == EAGAIN || errno == EINTR)) return;
    if(n <= 0) { if(g_echo_io) { sevent_io_unregister(g_ctx, g_echo_io); g_echo_io = NULL; }
                 close(g_echo_fd); g_echo_fd = 0; return; }
    g_echo_len += (size_t)n;
    ...
    while(pos < g_echo_len) {
        ...
        if(hlen == 0) break;
        if(hlen < 0) { g_echo_len = 0; return; }
        size_t fsize = (size_t)hlen + (size_t)hdr.payload_len;
        if(g_echo_len - pos < fsize) break;

        uint8_t *pld = g_echo_buf + pos + hlen;
        if(hdr.mask) ws_frame_apply_mask(pld, hdr.payload_len, hdr.mask_key);
    ...
    if(pos > 0) {
        if(pos < g_echo_len) memmove(g_echo_buf, g_echo_buf + pos, g_echo_len - pos);
        g_echo_len -= pos;
    }
}
...
static void on_echo_accept(void *data) {
    ...
    if(g_echo_io) { sevent_io_unregister(g_ctx, g_echo_io); g_echo_io = NULL; }
    g_echo_hs = 0; g_echo_len = 0;
    sevent_io_handler h = { .fd = g_echo_fd, .io_read = on_echo_read };
    g_echo_io = sevent_io_register(g_ctx, &h);
    if(!g_echo_io) { close(g_echo_fd); g_echo_fd = 0; }
}
...
static void on_ws_close(void *d, uint16_t c, const char *r, size_t l) {
    (void)d; (void)c; (void)r; (void)l;
    if(g_pass != 1)
        g_pass = (g_rxlen == strlen(TEST_MSG) &&
                  memcmp(g_rxbuf, TEST_MSG, g_rxlen) == 0) ? 1 : -1;
...
int main(void) {
    setbuf(stdout, NULL);
    g_ctx = sevent_create();
    if(!g_ctx) { printf("FAIL: ctx\n"); return 1; }
    sevent_ignore_sigpipe();
    printf("redirect_test ...\n");

    /* 重定向服务端 */
    g_redir_fd = tcp_listen(PORT_REDIR);
    if(g_redir_fd < 0) return 1;
    if(!sevent_io_register(g_ctx, &(sevent_io_handler){ .fd = g_redir_fd,
        .io_read = on_redir_accept })) { printf("FAIL: io redir\n"); return 1; }

    /* 回声服务端 */
    g_echo_listen = tcp_listen(PORT_ECHO);
    if(g_echo_listen < 0) return 1;
    if(!sevent_io_register(g_ctx, &(sevent_io_handler){ .fd = g_echo_listen,
        .io_read = on_echo_accept })) { printf("FAIL: io echo\n"); return 1; }

    /* WS 客户端 — 连的是重定向服务端 */
    g_ws = sevent_ws_connect(g_ctx, &(sevent_ws_config){
        .host = "127.0.0.1", .port = PORT_REDIR, .path = "/old-path",
        .on_open = on_ws_open, .on_message = on_ws_msg,
        .on_close = on_ws_close, .on_error = on_ws_err });
    if(!g_ws) { printf("  FAIL: connect\n"); return 1; }
