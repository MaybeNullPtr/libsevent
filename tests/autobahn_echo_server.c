...
static void compute_accept(const char *key, char *out, size_t out_cap) {
    ...
    if(n < 0 || (size_t)n >= sizeof(concat)) return;
 ...
 *  构造 WS 升级响应
 * --------------------------------------------------------------- */
static int build_101_response(const uint8_t *req, size_t req_len,
    ...
    while(p && p < end) {
        ...
        if(llen > strlen(key_hdr)) {
            ...
            if(match) {
                ...
                if(vlen > 0 && val_start[vlen - 1] == '\r') vlen--;
 ...
 * --------------------------------------------------------------- */
static void echo_payload(client_t *cl, const ws_frame_header *hdr,
...
static void on_client_read(void *data) {
    ...
    ssize_t n = read(cl->fd, cl->recv_buf + cl->recv_len,
                     sizeof(cl->recv_buf) - cl->recv_len);
    if(n <= 0) {
        /* 关闭连接 */
        if(cl->io) { sevent_io_unregister(g_ctx, cl->io); cl->io = NULL; }
        close(cl->fd);
        /* 从链表移除 */
        ...
        free(cl);
        if(g_quit && !g_clients) sevent_stop(g_ctx);
        return;
    }
    ...
    while(pos < cl->recv_len) {
        ...
        if(cl->recv_len - pos < frame_size) break;
...
static void on_accept(void *data) {
    ...
    if(!cl->io) { close(cfd); free(cl); return; }
...
int main(int argc, char **argv) {
    ...
    g_ctx = sevent_create();
    if(!g_ctx) { fprintf(stderr, "create fail\n"); return 1; }
    ...
    if(bind(g_listen_fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        perror("bind"); close(g_listen_fd); return 1;
    }
    if(listen(g_listen_fd, 5) < 0) {
        perror("listen"); close(g_listen_fd); return 1;
    }
    ...
    while(g_clients) {
        ...
        g_clients = cl->next;
        if(cl->io) sevent_io_unregister(g_ctx, cl->io);
        close(cl->fd);
        free(cl);
    }
    close(g_listen_fd);
