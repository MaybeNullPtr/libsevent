...
static int build_frame(uint8_t *buf, uint8_t fin, uint8_t opcode,
...
{
    uint8_t frame[] = { 0x89, 0x85, 0x01, 0x02, 0x03, 0x04,
...
TEST(frame_parse_large_length)
{
    ...
    buf[2] = 0x00; buf[3] = 0x00;        /* 64-bit 扩展 (Big-Endian) */
    buf[4] = 0x00; buf[5] = 0x00;
    buf[6] = 0x00; buf[7] = 0x01;
    buf[8] = 0x00; buf[9] = 0x17;        /* = 0x000117 = 70000 - 256 - 255 ... */
...
TEST(frame_build_small_masked)
{
    ...
    ASSERT_EQ(0x01, buf[2]); ASSERT_EQ(0x02, buf[3]);
    ASSERT_EQ(0x03, buf[4]); ASSERT_EQ(0x04, buf[5]);
}
...
TEST(frame_roundtrip_small)
{
    ...
    int payload_len = 6;
    uint8_t frame[32];
    int total = build_frame(frame, 1, WS_OPCODE_TEXT, key,
                             payload, payload_len);
    ASSERT(total > 0);
...
TEST(handshake_build_request_with_protocol)
{
    ...
    int n = ws_build_request(buf, sizeof(buf), "chat.example.com", 9000,
                              "/chat", key, "myprotocol");
    ASSERT_GT(n, 0);
...
static const char *sample_101_response =
...
/* 辅助: 构建不含协议头的 101 响应 */
static const char *sample_101_no_proto =
    "HTTP/1.1 101 Switching Protocols\r\n"
    "Upgrade: websocket\r\n"
    "Connection: Upgrade\r\n"
    "Sec-WebSocket-Accept: s3pPLMBiTxaQ9kYGzzhZRbK+xOo=\r\n"
...
TEST(handshake_parse_101_basic)
{
    ...
    int n = ws_parse_response((const uint8_t *)sample_101_response,
                               strlen(sample_101_response), &resp);
    ASSERT_GT(n, 0);
...
{
    ws_handshake_response resp;
    int n = ws_parse_response((const uint8_t *)sample_101_no_proto,
                               strlen(sample_101_no_proto), &resp);
    ASSERT_GT(n, 0);
    ASSERT_EQ(101, resp.status_code);
    ASSERT_EQ(0, strcmp(resp.accept, "s3pPLMBiTxaQ9kYGzzhZRbK+xOo="));
...
#define TEST_LIST                                                           \
    T(sha1_empty) T(sha1_abc) T(sha1_len_448) T(sha1_streaming)            \
    T(sha1_million_a) T(sha1_ws_key)                                       \
    T(base64_empty) T(base64_single) T(base64_double) T(base64_triple)     \
    T(base64_4bytes) T(base64_5bytes) T(base64_6bytes)                     \
    T(base64_binary) T(base64_insufficient_buffer) T(base64_just_fit)      \
    T(base64_encode_size)                                                   \
    T(frame_parse_small_unmasked) T(frame_parse_small_masked)               \
    T(frame_parse_medium_length) T(frame_parse_large_length)                \
    T(frame_parse_incomplete) T(frame_parse_incomplete_ext16)               \
    T(frame_parse_incomplete_ext64) T(frame_parse_incomplete_mask)          \
    T(frame_parse_rsv_error) T(frame_parse_msb_length_error)                \
    T(frame_parse_continuation)                                             \
    T(frame_build_small_unmasked) T(frame_build_small_masked)               \
    T(frame_build_medium) T(frame_build_large)                              \
    T(frame_build_invalid_opcode)                                           \
    T(frame_roundtrip_small)                                                \
    T(frame_apply_mask)                                                     \
    T(handshake_gen_key) T(handshake_build_request_basic)                   \
    T(handshake_build_request_with_protocol)                                \
    T(handshake_build_request_buffer_too_small)                             \
    T(handshake_parse_101_basic) T(handshake_parse_101_no_protocol)         \
    T(handshake_parse_non_101) T(handshake_parse_incomplete)                \
    T(handshake_parse_incremental)                                          \
    T(handshake_parse_case_insensitive)                                     \
    T(handshake_parse_missing_accept)                                       \
    T(handshake_verify_accept) T(handshake_verify_accept_wrong)             \
