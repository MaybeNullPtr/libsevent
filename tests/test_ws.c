    ...
    T(sha1_streaming) T(sha1_million_a) T(sha1_ws_key) T(base64_empty) T(base64_single) T(base64_double) T(                                                             \
            ...
            T(base64_insufficient_buffer) T(base64_just_fit) T(base64_encode_size) T(frame_parse_small_unmasked) T(                                                     \
                    ...
                    T(frame_parse_incomplete) T(frame_parse_incomplete_ext16) T(frame_parse_incomplete_ext64) T(                                                        \
                            frame_parse_incomplete_mask) T(frame_parse_rsv_error) T(frame_parse_msb_length_error)                                                       \
                            T(frame_parse_continuation) T(frame_build_small_unmasked) T(frame_build_small_masked) T(                                                    \
                                    frame_build_medium) T(frame_build_large) T(frame_build_invalid_opcode)                                                              \
                                    T(frame_roundtrip_small) T(frame_apply_mask) T(handshake_gen_key) T(                                                                \
                                            handshake_build_request_basic) T(handshake_build_request_with_protocol)                                                     \
